#include <opencv2/core/core.hpp>
#include <opencv2/highgui/highgui.hpp>
#include <opencv2/imgproc/imgproc.hpp>
#include <stdlib.h>
#include <float.h>
#include <stdio.h>
#include <vector>
#include <cstdint>
#include "AudioFFT.h"

#include "framework.h"

const unsigned char kColorPalette[768] = { 
  , 
 };

#define AUDIO_BUF_SIZE (1024)
#define FFT_SIZE (1024)

enum{
    kShiftUp = 1, 
    kShiftRight, 
    kShiftDown, 
    kShiftLeft
};

long map(long x, long in_min, long in_max, long out_min, long out_max) 
{
  return (x - in_min) * (out_max - out_min) / (in_max - in_min) + out_min;
}

cv::Mat shiftFrame(cv::Mat &frame, int pixels, int direction)
{
    //create a same sized temporary Mat with all the pixels flagged as invalid (-1)
    cv::Mat temp = cv::Mat::zeros(frame.size(), frame.type());

    switch (direction)
    {
    case(kShiftUp) :
        frame(cv::Rect(0, pixels, frame.cols, frame.rows - pixels)).copyTo(temp(cv::Rect(0, 0, temp.cols, temp.rows - pixels)));
        break;
    case(kShiftRight) :
        frame(cv::Rect(0, 0, frame.cols - pixels, frame.rows)).copyTo(temp(cv::Rect(pixels, 0, frame.cols - pixels, frame.rows)));
        break;
    case(kShiftDown) :
        frame(cv::Rect(0, 0, frame.cols, frame.rows - pixels)).copyTo(temp(cv::Rect(0, pixels, frame.cols, frame.rows - pixels)));
        break;
    case(kShiftLeft) :
        frame(cv::Rect(pixels, 0, frame.cols - pixels, frame.rows)).copyTo(temp(cv::Rect(0, 0, frame.cols - pixels, frame.rows)));
        break;
    default:
        std::cout << "Shift direction is not set properly" << std::endl;
    }

    return temp;
}

float factors[FFT_SIZE / 2];

void computeFactors(float exp)
{
    factors[0] = 0.01;
    for(int i = 1; i < (FFT_SIZE / 2); i++)
    {
        factors[i] = (float)log(pow(i, exp));
    }
}


int main(int argc, char **argv) 
{
    startFramework("Audio FFT", 640, 480, true);

    const char *arecord_cmd = "arecord -D plughw:0 -f S16_LE -c 1 -r 48000 -t raw -q -";
    int16_t audio_buf[AUDIO_BUF_SIZE];
    float audio_buf_f[AUDIO_BUF_SIZE];
    FILE *arecord_stdout = popen(arecord_cmd, "r");

    cv::Mat mat_out = cv::Mat::zeros(cv::Size(640, 480), CV_8UC3);
    
    int prev_rms = 0;
    audiofft::AudioFFT fft;
    fft.init(FFT_SIZE);

    DynamicJsonDocument doc(1024 * 16);

    computeFactors(0.1);

    while(true)
    {
        double t_start = ncnn::get_current_time();
        fread((char*)audio_buf, 2, AUDIO_BUF_SIZE, arecord_stdout);
        double t_read = ncnn::get_current_time();
        double sum = 0;
        for(int i = 0; i < AUDIO_BUF_SIZE; i++)
        {
            // audio_buf_f[i] = (float)audio_buf[i];
            audio_buf_f[i] = (float)map(audio_buf[i], 0, INT16_MAX, 0, 4000);
            sum += audio_buf[i] * audio_buf[i];
            // audio_buf_f[i] = (float)(map(audio_buf[i] >> 5, 0, INT16_MAX, 0, 4000));
        }
        double rms = sqrt(sum / AUDIO_BUF_SIZE);

        std::vector<float> re(audiofft::AudioFFT::ComplexSize(FFT_SIZE)); 
        std::vector<float> im(audiofft::AudioFFT::ComplexSize(FFT_SIZE)); 
        
        fft.fft(audio_buf_f, re.data(), im.data());
        double t_fft = ncnn::get_current_time();

        for(int i = 0; i < (FFT_SIZE / 2); i++)
        {
            float data = (float)sqrt(re[i] * re[i] + im[i] * im[i]);
            data *= factors[i];
            data = data > 4000.0f ? 4000.0f : data;
            // data = data > INT16_MAX ? INT16_MAX : data;
            int index = map((int)data, 0, 4000, 0, 255);
            index = index > 255 ? 255 : index;
            doc["meta"][i] = index;
        }
        // int mapped_rms = map((int)rms, 0, INT16_MAX, 0, 128);
        doc["rms"] = (int)rms;

        sendPIPEJsonDoc(doc);

        double t_end = ncnn::get_current_time();
        double dt_total = t_end - t_start;
        fprintf(stderr, "total %3.1f, t_read %3.1f, t_fft %3.1f\n", dt_total, t_read - t_start, t_fft - t_read);

        // double sum = 0;
        // for(int i = 0; i < AUDIO_BUF_SIZE; i++)
        // {
        //     sum += audio_buf[i] * audio_buf[i];
        // }
        // double rms = sqrt(sum / AUDIO_BUF_SIZE);
        // fprintf(stderr, "rms = %.2lf\n", rms);

        // int mapped_rms = map((int)rms, 0, INT16_MAX, 0, 240);

        // cv::line(mat_out, cv::Point(639, 240 - mapped_rms), cv::Point(639, 240 + mapped_rms), cv::Scalar(0, 255, 0), 1);
        // prev_rms = mapped_rms;

        // sendMat(mat_out);
    }

    pclose(arecord_stdout);
    return 0;
}